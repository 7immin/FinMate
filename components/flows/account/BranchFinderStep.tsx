...
import { BRANCH_DISTANCES } from "@/lib/mock/account";
...
export function BranchFinderStep({ onSelect }: { onSelect: (branchName: string) => void }) {
  const { t, tNode } = useTranslation();
  const branches = tNode<BranchContent[]>("account.branch.list");
  const nearest = branches[0];

  return (
    <div className="flex flex-1 flex-col">
      <TopBar title={t("account.branch.topBarTitle")} />
      <div className="flex-1 space-y-5 px-5 pb-6 pt-2">
        <div className="relative h-40 overflow-hidden rounded-2xl border border-border bg-[linear-gradient(rgba(255,255,255,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.05)_1px,transparent_1px)] bg-[length:24px_24px] bg-surface">
          <span className="absolute right-3 top-3 text-xs text-foreground-subtle">
            {t("account.branch.mapLabel")}
          </span>
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium text-foreground-muted">{t("account.branch.nearestTitle")}</p>
          <span className="text-xs text-foreground-subtle">{t("account.branch.filterLabel")}</span>
        </div>
          ...
          {branches.map((branch, idx) => (
            <Card key={branch.name}>
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-foreground-muted">{BRANCH_DISTANCES[idx]}</span>
              </div>
      ...
      <div className="px-5 pb-6">
        <Button onClick={() => onSelect(nearest.name)}>
          {t("account.branch.visitPrep", { name: nearest.name })}
        </Button>
      </div>
    </div>
  );
